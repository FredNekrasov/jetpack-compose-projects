...
class GetPWUseCase(
    ...
    fun find(value: String, sortingPW: SortingPW) = invoke(sortingPW).map { pwList ->
        pwList.fastFilter {
            it.pwName.lowercase().startsWith(value.lowercase())
                    || it.student.lowercase().startsWith(value.lowercase())
                    || it.variant.toString().startsWith(value)
                    || it.level.toString().startsWith(value)
                    || it.date.startsWith(value)
                    || it.mark.toString().startsWith(value)
        }
    }
    /**
