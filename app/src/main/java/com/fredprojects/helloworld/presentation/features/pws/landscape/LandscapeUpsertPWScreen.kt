package com.fredprojects.helloworld.presentation.features.pws.landscape
    ...
    Scaffold(
        floatingActionButton = {
            FredFloatingActionButton(
                onClick = {
                    onUpsert(PracticalWork(pwName, student, variant.toInt(), lvl.toInt(), date, mark.toInt(), photo))
                    when(state.status) {
        ...
        Column(Modifier.fillMaxWidth().padding(paddingValues), Arrangement.Center, Alignment.CenterHorizontally) {
            ...
            Spacer(Modifier.height(16.dp))
            Row(Modifier.fillMaxWidth().padding(paddingValues), Arrangement.SpaceEvenly, Alignment.CenterVertically) {
                Column(Modifier.fillMaxWidth().padding(paddingValues), Arrangement.Center, Alignment.CenterHorizontally) {
